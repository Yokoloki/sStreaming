...
class Switching(app_manager.RyuApp):
    ...
    def del_flow_by_id(self, flow_id):
        ...
        for entry in self.flows[flow_id]:
            ...
            match = entry["match"]
            self.del_switch_flow(dpid, self.dps[dpid].ofproto.OFPP_ANY, group_id, match)
    ...
    def _switching_handler(self, ev):
        ...
        paths = nx.all_shortest_paths(self.graph, source=dpid, target=dst_dpid)
        paths = list(paths)
        ...
        for i in xrange(path_len-1):
            ...
            for src_dpid in rules.keys():
                self.add_multiswitch_flow(flow_id, src_dpid, eth_dst, rules[src_dpid])
        self.add_multiswitch_flow(flow_id, dst_dpid, eth_dst, [dst_out_port])
    ...
    def add_multiswitch_flow(self, flow_id, dpid, eth_dst, out_ports):
